...
func (receiver *timingWheel) Add(d time.Duration, opts ...OpOption) *Timer {
	...
	flog2.Debugf("添加时间(%d):+%s %s 第%d层 第%d格 剩余%s，当前指针：%s", t.Id, t.duration.String(), t.PlanAt.Format("15:04:05.000"), level, curLevelTimeHand, t.remainingDuration.String(), builder.String())
	...
	for {
		...
		go func(timers []*Timer) {
			for i := 0; i < len(timers); i++ {
				if !timers[i].isStop {
					flog2.Debugf("休眠时间(%d):+%s %d us", timers[i].Id, timers[i].PlanAt.Format("15:04:05.000"), timers[i].PlanAt.Sub(time.Now()).Microseconds())
...
func (receiver *timingWheel) popTimer(timer *Timer) {
	...
	timer.C <- time.Now()
	flog2.Debugf("推送时间(%d):+%s 精确度：%v", timer.Id, timer.PlanAt.Format("15:04:05.000"), timer.isPrecision)
}
